...
const setGridStyle = (grid, portionX, portionY) => {
  ...
  style.innerText = `.maze-generator-cell{position:relative}.maze-generator-cell::before{transform:scale(1.41)rotate(var(--deg));content:"";display:block;width:1px;background-color:black;margin:0 auto;height:100%;transition:all var(--second);will-change:transform}`;
...
// create
const createMaze = ({ el = "#app", gridSize = 10, cellSize = 2 }) => {
  ...
  setGridStyle(grid, portionX, portionY);
